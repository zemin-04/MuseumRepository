...
public class CommonController {
	...
	@RequestMapping("/home")
	public String home(String code) throws JsonParseException, JsonMappingException, IOException{
		System.out.println("code:" + code);
		String url = String.format("https://api.weixin.qq.com/sns/oauth2/access_token?appid=%s&secret=%s&code=%s&grant_type=authorization_code", WeiXinConfigConstant.APP_ID, WeiXinConfigConstant.APP_SECRET, code);
		...
		if(null == user){
			String url2 = String.format("https://api.weixin.qq.com/sns/userinfo?access_token=%s&openid=%s&lang=zh_CN", oauthToken.getAccessToken(), oauthToken.getOpenid());
			String result2 = HttpClientUtil.httpGetRequest(url2);
			System.out.println(result2);
			user = objectMapper.readValue(result2, User.class);
			userService.insertUser(user);
		}
		System.out.println(user.getNickname() + "'" + Arrays.toString(user.getPrivilege()));
		return "index";
	}
}
